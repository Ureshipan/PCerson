...
Там же теперь выбирается локальный backend для голосового ввода. По умолчанию это `Vosk Local`: bootstrap ставит `vosk + sounddevice` и скачивает офлайн-модель автоматически.
...
В overlay доступна диктовка: `Ctrl+4` запускает локальное распознавание речи и подставляет распознанный текст в поле ввода.
